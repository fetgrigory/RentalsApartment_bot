...
async def delete_apartment(callback_query: types.CallbackQuery, state: FSMContext):
    data = await state.get_data()
    apartments = data.get("apartments", [])

    index = int(callback_query.data.split("_")[-1])

    if apartments and index < len(apartments):
        apartment = apartments[index]

        delete_apartment_data(apartment.id)

        data["apartments"] = get_catalog_data()
        data["current_apartment"] = None
